    ...
    return (
        ...
        >
            ...
            <div className="p-4 flex flex-col flex-1">
                <p className="text-xs text-gray-500 mb-1">{book.categories.map((c) => c.name).join(' / ')}</p>
                <h3 className="font-semibold text-gray-800 line-clamp-2 flex-1">{book.title}</h3>
                <p className="text-sm text-gray-500 mt-1">{book.author}</p>
                <p className="text-indigo-600 font-bold mt-2">¥{book.price.toLocaleString()}</p>
            </div>
        </Link>
    );
}
        ...
        <>
            ...
            <MainLayout>
                ...
                <section className="bg-indigo-600 text-white py-20 px-4 text-center">
                    <h1 className="text-4xl font-bold mb-4">子供の本の世界へ</h1>
                    <p className="text-indigo-200 mb-8 text-lg">年齢・ジャンルから、ぴったりの一冊を見つけよう</p>
                    <Link
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    {newArrivals.length > 0 && (
                        <section className="mt-16">
                            <div className="flex items-center justify-between mb-6">
                                <h2 className="text-2xl font-bold text-gray-800">新着本</h2>
                                <Link href={route('books.index')} className="text-indigo-600 text-sm hover:underline">
                                    すべて見る →
                                </Link>
                            </div>
                    ...
                    {categories.length > 0 && (
                        <section className="mt-16 mb-8">
                            <h2 className="text-2xl font-bold text-gray-800 mb-6">カテゴリ別に探す</h2>
